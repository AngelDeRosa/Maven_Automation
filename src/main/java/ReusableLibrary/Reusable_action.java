...
//Method to type on a webelement
    public static void sendKeys(WebDriver driver,String xpathLocator,ExtentTest logger, String elementName){
        //define by explict wait
        WebDriverWait wait= new WebDriverWait(driver,10);
        //Use try catch to locate an element and click on it
        try {
            WebElement element= wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(xpathLocator)));
            element.clear();
            element.sendKeys();
            logger.log(LogStatus.PASS,"Successfully type a value on " + elementName);
        }catch (Exception e){
            logger.log(LogStatus.FAIL,"Unable to type on element " + elementName + " " + e);
            getScreenShot(driver,elementName,logger);
            System.out.println("Unable to type in on element" + elementName + " " + e);
        }//end of sendkeys method
    }
